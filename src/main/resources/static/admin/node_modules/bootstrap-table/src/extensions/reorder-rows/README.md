...
* type: Function
* description: Pass a function that will be called when the student starts dragging. The function takes 2 parameters: the table and the row which the student has started to drag.
...
* After search if the student reorder the rows the data is not shown properly after that.